        ...
        <article className="group cursor-pointer mb-14">
          ...
          <div className="mt-5 max-w-5xl">
            ...
            <div className="mt-5 pt-4 border-t border-gray-200 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-gray-400">
              <span className="font-semibold text-gray-600">By {hero.author}</span>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[650px] text-sm border-collapse">
              ...
              <tbody className="divide-y divide-gray-100">
                {earningsNews.map((e) => (
                  <tr key={e.ticker} className="transition-colors hover:bg-gray-50">
                    <td className="py-3.5 pr-4">
                      <span className="font-semibold text-gray-900">{e.company}</span>
                      <span className="ml-1.5 text-gray-400 text-xs">({e.ticker})</span>
                    </td>
              ...
              <tbody className="divide-y divide-gray-100">
                {maDeals.map((d) => (
                  <tr key={d.id} className="transition-colors hover:bg-gray-50">
                    <td className="py-3.5 pr-4 font-semibold text-gray-900">{d.acquirer}</td>
                    <td className="px-3 py-3.5 text-gray-600">{d.target}</td>
                    <td className="px-3 py-3.5 text-right font-bold tabular-nums text-gray-900">{d.value}</td>
                    <td className="hidden px-3 py-3.5 text-xs text-gray-500 md:table-cell">{d.sector}</td>
                    <td className="py-3.5 pl-3 text-right">
                      <span
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-10 border-t-2 border-black pt-10">
          ...
          <div>
            ...
            <div className="divide-y divide-gray-200">
              {corporateNews.map((n) => (
                <article key={n.id} className="group cursor-pointer py-4 first:pt-0">
                  <span className="text-[11px] font-bold uppercase tracking-[0.14em] text-red-600">
                    {n.category}
                  </span>

                  <h3 className="mt-1.5 text-sm md:text-[15px] font-semibold leading-[1.5] text-gray-900 transition-colors group-hover:text-red-600">
                    {n.title}
                  </h3>

                  <span className="mt-2 flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-gray-400">
          ...
          <div>
            ...
            <div className="divide-y divide-gray-200">
              {startupNews.map((n) => (
                <article key={n.id} className="group cursor-pointer py-4 first:pt-0">
                  <h3 className="text-sm md:text-[15px] font-semibold leading-[1.5] text-gray-900 transition-colors group-hover:text-red-600">
                    {n.title}
                  </h3>
